...
func makePlan(kind: SessionKind, options: Options) -> SessionPlan {
    ...
    guard let rounds = options.rounds else {
        return SessionPlan.standard(for: kind, baselines: baselines)
    }
    switch kind {
    case .co2Table: return .co2Table(maxHold: options.maxHold, parameters: CO2TableParameters(rounds: rounds))
    case .o2Table: return .o2Table(maxHold: options.maxHold, parameters: O2TableParameters(rounds: rounds))
    case .cadence: return .cadence(bolt: options.bolt, parameters: CadenceParameters(rounds: rounds))
    case .hypoxicSprints: return .hypoxicSprints(maxHold: options.maxHold, parameters: HypoxicSprintParameters(reps: rounds))
    case .imt: return .imt(parameters: IMTParameters(sets: rounds))
    default: return SessionPlan.standard(for: kind, baselines: baselines)
    }
}
